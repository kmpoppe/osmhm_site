...
def key_watch(request):
    try:
        ...
        full_history = DBSession.query(History_Keys).filter(
            Watched_Keys.authorid == userid).join(
            Watched_Keys,
            History_Keys.wid == Watched_Keys.id).order_by(
            desc(
                History_Keys.changeset)).all()
        history = DBSession.query(History_Keys).filter(
            Watched_Keys.authorid == userid).join(
            Watched_Keys,
            History_Keys.wid == Watched_Keys.id).distinct(
            History_Keys.username,
            History_Keys.key,
            History_Keys.value).all()
        ...
        for event in full_history:
            ...
            if event.changeset not in changesets[event.username][event.key][event.value]:
                changesets[event.username][event.key][event.value].append(
                    event.changeset)
                changeset_strs[event.username][event.key][event.value] += ('<a href="https://www.openstreetmap.org/changeset/%s" target="_blank">%s</a> (%s), ' % (
