...
def object_watch(request):
    try:
        ...
        history = DBSession.query(
            History_Objects,
            Watched_Objects).filter(
            Watched_Objects.authorid == userid).join(
            Watched_Objects,
            History_Objects.wid == Watched_Objects.id).order_by(
                desc(
...
def object_watch_event_delete(request):
    DBSession.query(History_Objects).filter_by(
        id=request.matchdict['id']).delete()
    DBSession.flush()
    ...
    if request.method == 'POST':
        ...
        if userid:
            ...
            objectToAdd = Watched_Objects(author=user.username,
                                          authorid=userid,
                                          element=request.POST.getone(
                                              'addobjectname'),
                                          reason=request.POST.getone(
                                              'addreason'),
