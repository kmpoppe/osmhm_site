...
def user_watch(request):
    try:
        ...
        history = DBSession.query(
            History_Users,
            Watched_Users).filter(
            Watched_Users.authorid == userid).join(
            Watched_Users,
            History_Users.wid == Watched_Users.id).order_by(
                desc(
...
def user_watch_event_delete(request):
    DBSession.query(History_Users).filter_by(
